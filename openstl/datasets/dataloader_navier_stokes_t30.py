...
class NavierStokesT30Dataset(Dataset):
    
    def __init__(self, data_root, split='train', image_size=64,
                 pre_seq_length=10, aft_seq_length=10, step=1, use_augment=False, channels=1, train_ratio=0.8):
        super(NavierStokesT30Dataset, self).__init__()
...
def load_data(batch_size, val_batch_size, data_root, num_workers=4,
    ...
    data_root = os.path.join(data_root, 'navier_stokes_t30')
    image_size = in_shape[-1] if len(in_shape) >= 3 else 64
    channels = in_shape[1] if len(in_shape) > 1 else 1
    train_set = NavierStokesT30Dataset(data_root, split='train', image_size=image_size,
                                       pre_seq_length=pre_seq_length, aft_seq_length=aft_seq_length,
                                       step=1, use_augment=use_augment, channels=channels)
    test_set = NavierStokesT30Dataset(data_root, split='test', image_size=image_size,
                                      pre_seq_length=pre_seq_length, aft_seq_length=aft_seq_length,
